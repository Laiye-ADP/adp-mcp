...
#### Option A: Visual Settings UI (Recommended)

Most MCP clients (Cursor, Tongyi Lingma, Coze, etc.) let you add MCP servers directly from their settings panel. Using Cursor as an example:

1. Open **Settings → MCP**
2. Click **Add new MCP server**
3. Fill in the following:
   - **Name**: `ADP` (any name you like)
   - **Type**: `streamable-http`
   - **URL**: `https://adp.laiye.com/mcp?key=<YOUR-ADP-API-Key>`
4. Save — ADP tools will be available in your next conversation
...
| **Transport** | Streamable HTTP |
| **Authentication** | URL query param `?key=<YOUR-ADP-API-Key>` |

> For private deployments, replace the domain with your actual server address.
...
ADP MCP Server uses API Key authentication, passed via URL query parameter:
...
- API Key never appears in request bodies or responses
